...
def generate_synthetic_predictions(
    ...
    peak_positions = np.random.choice(
        range(peak_width, size - peak_width), size=num_peaks, replace=False
    )
...
def generate_variant_predictions_1d(
    size: int, svtype: str, variant_bin: int, svlen_bins: int, num_peaks: int = 6
) -> tuple:
    """
    ...
    if svtype == "INS":
        ...
        alt_pred = np.concatenate(
            [alt_base[:variant_bin], insert_region, alt_base[variant_bin:]]
        )
        ...
        ref_pred = np.concatenate(
            [ref_base[:variant_bin], insert_region, ref_base[variant_bin:]]
        )
...
def generate_synthetic_contact_map(
    size: int, decay_rate: float = 0.05, noise: float = 0.1, seed: int = 42
) -> np.ndarray:
    """
...
def visualize_1d_alignment(
    ...
    axes[1].plot(
        bins_aligned, ref_aligned, "b-", linewidth=2, label="REF (aligned)", alpha=0.8
    )
    axes[1].plot(
        bins_aligned, alt_aligned, "r-", linewidth=2, label="ALT (aligned)", alpha=0.8
    )
    axes[1].axvline(
        variant_bin,
        color="purple",
    ...
    fig.suptitle(
        f"1D Prediction Alignment: {variant_type} at bin {variant_bin}",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def visualize_2d_alignment(
    ...
    im0 = axes[0, 0].imshow(
        ref_orig, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper", aspect="auto"
    )
    axes[0, 0].set_title("Original REF", fontsize=11, fontweight="bold")
    axes[0, 0].axvline(
        variant_bin, color="blue", linestyle="--", linewidth=1, alpha=0.6
    )
    axes[0, 0].axhline(
        variant_bin, color="blue", linestyle="--", linewidth=1, alpha=0.6
    )
    ...
    im1 = axes[0, 1].imshow(
        alt_orig, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper", aspect="auto"
    )
    ...
    im2 = axes[1, 0].imshow(
        ref_aligned, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper", aspect="auto"
    )
    axes[1, 0].set_title(
        "Aligned REF", fontsize=11, fontweight="bold", color="darkblue"
    )
    ...
    axes[1, 0].axhline(
        variant_bin, color="blue", linestyle="--", linewidth=1.5, alpha=0.8
    )
    ...
    im3 = axes[1, 1].imshow(
        alt_aligned, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper", aspect="auto"
    )
    ...
    fig.suptitle(
        f"2D Prediction Alignment: {variant_type} at bin {variant_bin}",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
...
def visualize_bnd_1d(
    left_ref: np.ndarray,
    right_ref: np.ndarray,
    bnd_alt: np.ndarray,
    chimeric_ref: np.ndarray,
    breakpoint_bin: int,
    output_path: str,
) -> None:
    """
    ...
    axes[0, 1].axvspan(
        breakpoint_bin, len(bins), alpha=0.1, color="green", label="Used region"
    )
    ...
    axes[1, 0].plot(
        bins, chimeric_ref, "b-", linewidth=2, label="Chimeric REF", alpha=0.8
    )
    axes[1, 0].axvline(
        breakpoint_bin,
        color="red",
    ...
    axes[1, 0].set_title(
        "Chimeric REF (Assembled)", fontsize=12, fontweight="bold", color="darkblue"
    )
    axes[1, 0].set_xlabel("Bin position")
    ...
    ax_aligned.plot(
        bins, chimeric_ref, "b-", linewidth=2, label="Chimeric REF (aligned)", alpha=0.8
    )
    ax_aligned.plot(
        bins, bnd_alt, "r-", linewidth=2, label="BND ALT (original)", alpha=0.8
    )
    ...
    ax_aligned.set_title(
        "Aligned Predictions Comparison", fontsize=12, fontweight="bold"
    )
    ...
    fig.suptitle(
        f"1D Prediction Alignment: BND at bin {breakpoint_bin}\n"
...
def visualize_bnd_2d(
    ...
    vmin = min(
        np.nanmin(left_ref),
        np.nanmin(right_ref),
        np.nanmin(chimeric_ref),
        np.nanmin(bnd_alt),
    )
    vmax = max(
        np.nanmax(left_ref),
        np.nanmax(right_ref),
        np.nanmax(chimeric_ref),
        np.nanmax(bnd_alt),
    )
    ...
    im1 = ax1.imshow(
        left_ref_flipped, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper"
    )
    ...
    im2 = ax2.imshow(
        right_ref_flipped, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper"
    )
    ...
    rect2 = patches.Rectangle(
        (rect2_x, breakpoint_bin),
        matrix_size - breakpoint_bin,
        matrix_size - breakpoint_bin,
        linewidth=3,
        edgecolor="green",
        facecolor="none",
    ...
    im5 = ax5.imshow(
        chimeric_ref_flipped, cmap="Reds", vmin=vmin, vmax=vmax, origin="upper"
    )
    ax5.set_title(
        "Chimeric REF (Aligned)", fontsize=12, fontweight="bold", color="darkred"
    )
    ...
    fig.suptitle(
        f"2D Prediction Alignment: BND at bin {breakpoint_bin}\n"
...
def main():
    ...
    aligner_2d = PredictionAligner2D(
        target_size=TARGET_SIZE, bin_size=BIN_SIZE, diag_offset=DIAG_OFFSET
    )
    ...
    # 2D predictions
    ref_matrix_ins = (
        generate_synthetic_contact_map(TARGET_SIZE, decay_rate=0.05, seed=42) * 0.5
    )
    alt_matrix_ins = generate_synthetic_contact_map(
        TARGET_SIZE, decay_rate=0.05, seed=42
    )
    ...
    # 2D
    ref_matrix_del = (
        generate_synthetic_contact_map(TARGET_SIZE, decay_rate=0.05, seed=43) * 0.5
    )
    alt_matrix_del = generate_synthetic_contact_map(
        TARGET_SIZE, decay_rate=0.05, seed=43
    )
    ...
    # 2D
    ref_matrix_dup = (
        generate_synthetic_contact_map(TARGET_SIZE, decay_rate=0.05, seed=44) * 0.5
    )
    alt_matrix_dup = generate_synthetic_contact_map(
        TARGET_SIZE, decay_rate=0.05, seed=44
    )
    ...
    # 2D
    ref_matrix_inv = (
        generate_synthetic_contact_map(TARGET_SIZE, decay_rate=0.05, seed=45) * 0.5
    )
    alt_matrix_inv = generate_synthetic_contact_map(
        TARGET_SIZE, decay_rate=0.05, seed=45
    )
    ...
    bnd_alt[:breakpoint_bin] = left_full[
        :breakpoint_bin
    ]  # Left locus pattern (full amplitude)
    bnd_alt[breakpoint_bin:] = right_full[
        breakpoint_bin:
    ]  # Right locus pattern (full amplitude)
    ...
    right_full_2d = generate_synthetic_contact_map(
        TARGET_SIZE, decay_rate=0.05, seed=51
    )
    ...
    bnd_alt_2d[:breakpoint_bin, :breakpoint_bin] = left_full_2d[
        :breakpoint_bin, :breakpoint_bin
    ]
    # Lower-right quadrant from right locus (full amplitude)
    bnd_alt_2d[breakpoint_bin:, breakpoint_bin:] = right_full_2d[
        breakpoint_bin:, breakpoint_bin:
    ]
    # Upper-right and lower-left quadrants: blend both loci to show trans-interactions (full amplitude)
    bnd_alt_2d[:breakpoint_bin, breakpoint_bin:] = (
        left_full_2d[:breakpoint_bin, breakpoint_bin:]
        + right_full_2d[:breakpoint_bin, breakpoint_bin:]
    ) / 2
    bnd_alt_2d[breakpoint_bin:, :breakpoint_bin] = (
        left_full_2d[breakpoint_bin:, :breakpoint_bin]
        + right_full_2d[breakpoint_bin:, :breakpoint_bin]
