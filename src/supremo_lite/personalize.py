    ...
    for chrom, chrom_vars in variants.groupby('chrom'):
        ...
        for _, var in chrom_vars.iterrows():
            ...
            if personal_seq[pos:pos+len(ref)] != ref:
                warnings.warn(f"Reference allele mismatch at {chrom}:{var['pos']}. Expected {ref}, found {personal_seq[pos:pos+len(ref)]}.")
                continue
...
def get_personal_sequences(reference_fn, variants_fn, seq_len):
    ...
    for _, var in variants.iterrows():
        ...
        if hasattr(reference_fn, '__getitem__'):  # Dictionary-like
            ...
            warnings.warn("File-based sequence extraction not implemented yet. Returning N's.")
...
def get_pam_disrupting_personal_sequences(reference_fn, variants_fn, seq_len, max_pam_distance, pam_sequence='NGG'):
    """
    ...
    for _, var in variants.iterrows():
        ...
        for i in range(len(ref_seq) - len(pam_sequence) + 1):
            ...
            if all(a == b or b == 'N' for a, b in zip(potential_pam.upper(), pam_sequence.upper())):
                pam_sites.append(i)
        
        # Filter PAM sites that are within max_pam_distance of the variant
        variant_pos_in_window = pos - window_start
        nearby_pam_sites = [p for p in pam_sites if abs(p - variant_pos_in_window) <= max_pam_distance]
        
        if nearby_pam_sites:
            pam_disrupting_variants.append(var)
            
            # Create sequence with the variant but PAM intact
            variant_seq = ref_seq[:variant_pos_in_window-1] + alt + ref_seq[variant_pos_in_window-1+len(ref):]
            ...
            for pam_site in nearby_pam_sites:
                ...
                disrupted_seq = variant_seq[:pam_site] + 'NNN' + variant_seq[pam_site+3:]  # Disrupt with NNN
                pam_disrupted_sequences.append((chrom, window_start, window_end, disrupted_seq))
