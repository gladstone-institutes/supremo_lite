...
def create_chromosome_mapping(reference_chroms: Set[str], vcf_chroms: Set[str]) -> Dict[str, str]:
    """
...
def get_chromosome_match_report(reference_chroms: Set[str], vcf_chroms: Set[str], 
                              mapping: Dict[str, str], unmatched: Set[str]) -> str:
    """
    ...
    report_lines.append(f"Reference chromosomes ({len(reference_chroms)}): {sorted(reference_chroms)}")
    ...
    report_lines.append(f"Matching coverage: {coverage:.1f}% ({len(mapping)}/{len(vcf_chroms)})")
    
    return "\n".join(report_lines)


def match_chromosomes_with_report(reference_chroms: Set[str], vcf_chroms: Set[str], 
                                verbose: bool = True) -> Tuple[Dict[str, str], Set[str]]:
    """
    ...
    if verbose and (len(mapping) < len(vcf_chroms) or 
                   any(k != v for k, v in mapping.items())):
        report = get_chromosome_match_report(reference_chroms, vcf_chroms, mapping, unmatched)
        print(report)
