...
def get_sm_sequences(chrom, start, end, reference_fasta):
    ...
    metadata_df = pd.DataFrame(
        metadata, 
        columns=['chrom', 'start', 'end', 'offset', 'ref', 'alt']
    )
...
def get_sm_subsequences(chrom, anchor, anchor_radius, seq_len, reference_fasta):
    """
    Generate sequences with all alternate nucleotides at positions around an anchor (saturation mutagenesis).
    ...
    assert len(ref_seq) == seq_len, f"Expected sequence length {seq_len}, got {len(ref_seq)}"
