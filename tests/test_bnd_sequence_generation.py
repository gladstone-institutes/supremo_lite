...
class TestBNDSequenceGeneration:
    ...
    def test_get_alt_ref_sequences_with_derived_inv(self):
        ...
        assert (
            sv_inv_chunk is not None
        ), "Should find chunk with SV_INV variants derived from BND preprocessing"
        ...
        assert not isinstance(
            ref_sequences, tuple
        ), "Synthetic SV_INV should have single reference structure"
    ...
    def test_get_alt_ref_sequences_with_derived_dup(self):
        ...
        assert (
            sv_dup_chunk is not None
        ), "Should find chunk with SV_DUP variants derived from BND preprocessing"
        ...
        assert not isinstance(
            ref_sequences, tuple
        ), "Synthetic SV_DUP should have single reference structure"
        assert isinstance(ref_sequences, list)
        assert len(ref_sequences) > 0
        # Extract sequence strings from tuples
        assert all(isinstance(seq, tuple) for seq in ref_sequences)
    ...
    def test_get_alt_ref_sequences_sv_types_comparison(self):
        ...
        test_cases = [
            (
                "tests/data/inv/inv.vcf",
                "SV_INV",
                False,
            ),  # Synthetic from BND preprocessing, single ref structure
            (
                "tests/data/dup/dup.vcf",
                "SV_DUP",
                False,
            ),  # Synthetic from BND preprocessing, single ref structure
            (
                "tests/data/bnd/bnd.vcf",
                "SV_BND",
                True,
            ),  # True breakends, dual ref structure
        ]
        ...
        for vcf_path, expected_variant_type, should_have_dual_refs in test_cases:
            ...
            ), f"Should find chunk with {expected_variant_type} variants in {vcf_path}"
            ...
            if expected_variant_type == "SV_BND":
                ...
                assert all(isinstance(seq, tuple) for seq in alt_sequences)
                seq_strings = [
                    seq[3] for seq in alt_sequences
            ...
            if should_have_dual_refs:
                assert isinstance(
                    ref_sequences, tuple
                ), f"{expected_variant_type} should have dual reference structure"
                assert (
                    len(ref_sequences) == 2
                ), f"{expected_variant_type} should have exactly 2 reference arrays"
                left_refs, right_refs = ref_sequences
                assert len(left_refs) > 0
                assert len(right_refs) > 0
                ...
                assert not isinstance(
                    ref_sequences, tuple
                ), f"{expected_variant_type} should NOT have dual reference structure"
                assert len(ref_sequences) > 0
            ...
            assert (
                expected_variant_type in variant_types
            ), f"Should have {expected_variant_type} in metadata"
            ...
            results_summary[expected_variant_type] = {
                ...
                "variant_count": len(
                    metadata[metadata["variant_type"] == expected_variant_type]
                ),
        ...
        assert (
            results_summary["SV_INV"]["ref_structure"] == "single"
        )  # Synthetic from BND preprocessing
        assert (
            results_summary["SV_DUP"]["ref_structure"] == "single"
        )  # Synthetic from BND preprocessing
        assert (
            results_summary["SV_BND"]["ref_structure"] == "dual"
        )  # True breakends/translocations
        ...
        for vcf_path, expected_variant_type, should_have_dual_refs in test_cases:
            ...
            if should_have_dual_refs:
                assert isinstance(
                    ref_encoded, tuple
                ), f"Encoded {expected_variant_type} should maintain dual reference structure"
