        ...
        for ref_seqs, alt_seqs, metadata in results:
            if metadata["variant_type"].iloc[0] == "SV_BND_INV":
                for idx, row in metadata.iterrows():
                    ...
                    assert (
                        window_start >= 0
                    ), f"Window start should be non-negative: {window_start}"
                    assert window_end <= len(
                        reference[chrom]
        ...
        for ref_seqs, alt_seqs, metadata in results:
            if metadata["variant_type"].iloc[0] == "SV_BND_INV":
                ...
                if isinstance(ref_seqs, list):
                    assert all(
                        len(seq) > 0 for seq in ref_seqs
                    ), "Sequences should not be empty"
                if isinstance(alt_seqs, list):
                    assert all(
                        len(seq) > 0 for seq in alt_seqs
                    ), "Sequences should not be empty"
                ...
                if isinstance(ref_seqs, list):
                    for seq in ref_seqs:
                        if "N" in seq:
                            ...
                            assert seq.startswith("N") or seq.endswith(
                                "N"
            ...
            for idx, row in metadata.iterrows():
                if row["alt"].startswith("<"):
                    ...
                    if "sym_variant_end" in row.index:
                        ...
                        assert (
                            calculated_svlen >= 0
                        ), "SVLEN should be non-negative for DUP"
        ...
        for ref_seqs, alt_seqs, metadata in results:
            if "DUP" in metadata["variant_type"].iloc[0]:
                for idx, row in metadata.iterrows():
                    ...
                    assert (
                        window_end > 0
                    ), f"Window end should be positive: {window_end}"
        ...
        if len(bnd_results) > 0:
            for ref_seqs, alt_seqs, metadata in bnd_results:
                for idx, row in metadata.iterrows():
                    ...
                    assert (
                        row["mate_pos"] > 0
                    ), f"Mate position should be positive: {row['mate_pos']}"
    ...
    def test_bnd_reference_tuple_structure(self):
        ...
        if len(bnd_results) > 0:
            for ref_seqs, alt_seqs, metadata in bnd_results:
                ...
                assert isinstance(
                    ref_seqs, (tuple, list)
                ), "BND ref sequences should be tuple or list"

                if isinstance(ref_seqs, tuple):
                    assert (
                        len(ref_seqs) == 2
                    ), "BND tuple should have (left_refs, right_refs)"
                    ...
                    # Should have same number of sequences
                    assert len(left_refs) == len(
                        right_refs
                    ), "Left and right refs should match"
...
class TestErrorHandling:
    ...
    def test_missing_end_field_for_symbolic_allele(self):
        ...
        for idx, var in symbolic_variants.iterrows():
            assert (
                "SV_" in var["variant_type"]
            ), "Symbolic alleles should be classified as SV"
    ...
    def test_overlapping_sv_detection(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert (
                len(personal_genome) > 0
            ), "Should process VCF with overlapping variants"
