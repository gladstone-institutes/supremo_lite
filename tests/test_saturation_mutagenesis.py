...
class TestSaturationMutagenesis(unittest.TestCase):
    ...
    def test_get_sm_sequences(self):
        ...
        ref_1h, alt_seqs, metadata = sl.get_sm_sequences(chrom, start, end, self.reference)
        ...
        self.assertEqual(list(metadata.columns), ['chrom', 'start', 'end', 'offset', 'ref', 'alt'])
        ...
        self.assertTrue(all(count == 3 for count in pos_counts))  # each should have 3 alternatives
    ...
    def test_get_sm_subsequences(self):
        ...
        ref_1h, alt_seqs, metadata = sl.get_sm_subsequences(chrom, anchor, anchor_radius, seq_len, self.reference)
        ...
        anchor_offset = anchor - (anchor - seq_len // 2)  # Anchor position in the sequence
