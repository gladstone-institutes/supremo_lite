...
class TestChunkedVCFReading:
    ...
    def test_read_vcf_chunked_single_chunk(self):
        ...
        try:
            ...
            assert list(chunks[0].columns) == ['chrom', 'pos', 'id', 'ref', 'alt']
            assert chunks[0]['chrom'].iloc[0] == 'chr1'
            assert chunks[0]['pos'].iloc[0] == 1000
        finally:
            os.unlink(vcf_path)
    ...
    def test_read_vcf_chunked_multiple_chunks(self):
        ...
        try:
            ...
            assert all_variants['pos'].iloc[0] == 1000
            assert all_variants['pos'].iloc[6] == 1600
        finally:
            os.unlink(vcf_path)
...
class TestChunkedPersonalSequences:
    """Test chunked processing in get_personal_sequences."""
    ...
    def test_chunked_processing_default_chunk_size(self):
        ...
        try:
            ...
            for result in results:
                assert result.shape == (1, seq_len, 4)  # (1 variant, seq_len, 4 nucleotides)
        finally:
            os.unlink(vcf_path)
    ...
    def test_chunked_processing_multiple_variants(self):
        ...
        try:
            ...
            assert results[1].shape == (2, seq_len, 4)  # array_split creates approximately equal chunks
