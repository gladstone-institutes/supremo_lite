...
class TestBNDClassificationAccuracy(unittest.TestCase):
    ...
    def test_inv_vcf_classification(self):
        ...
        self.assertEqual(len(bnd_variants), 4, "inv.vcf should contain exactly 4 BND variants")
        ...
        self.assertEqual(len(bnd_inv_variants), 4,
        ...
        ] if len(grouped['dup_variants']) > 0 else []
        self.assertEqual(len(bnd_dup_variants), 0,
                        "inv.vcf BND variants should NOT be classified as SV_BND_DUP")

        remaining_bnd_variants = grouped['bnd_variants'][
            grouped['bnd_variants']['variant_type'] == 'SV_BND'
        ] if len(grouped['bnd_variants']) > 0 else []
        self.assertEqual(len(remaining_bnd_variants), 0,
        ...
        for _, variant in bnd_inv_variants.iterrows():
            self.assertEqual(variant['chrom'], 'chr2',
                           f"All inv.vcf BND variants should be on chr2, found: {variant['chrom']}")
            self.assertEqual(variant['variant_type'], 'SV_BND_INV',
    ...
    def test_dup_vcf_classification(self):
        ...
        bnd_variants = variants_df[variants_df['variant_type'].isin(['SV_BND', 'SV_BND_INS'])]
        self.assertEqual(len(bnd_variants), 2, "dup.vcf should contain exactly 2 BND variants")
        ...
        self.assertEqual(len(bnd_dup_variants), 2,
                        "dup.vcf should have exactly 2 BND variants classified as SV_BND_DUP")

        # Verify no BND variants are misclassified
        bnd_inv_variants = grouped['inv_variants'][
            grouped['inv_variants']['variant_type'] == 'SV_BND_INV'
        ] if len(grouped['inv_variants']) > 0 else []
        self.assertEqual(len(bnd_inv_variants), 0,
        ...
        for _, variant in bnd_dup_variants.iterrows():
            ...
            self.assertEqual(variant['variant_type'], 'SV_BND_DUP',
    ...
    def test_bnd_vcf_classification(self):
        ...
        true_bnd_variants = grouped['bnd_variants'][
            grouped['bnd_variants']['variant_type'].isin(['SV_BND', 'SV_BND_INS'])
        ] if len(grouped['bnd_variants']) > 0 else []
        self.assertGreater(len(true_bnd_variants), 0,
                          "bnd.vcf should have BND variants remaining as SV_BND")

        # Verify no BND variants are misclassified
        misclassified_dup = grouped['dup_variants'][
            grouped['dup_variants']['variant_type'] == 'SV_BND_DUP'
        ] if len(grouped['dup_variants']) > 0 else []
        self.assertEqual(len(misclassified_dup), 0,
                        "bnd.vcf BND variants should NOT be misclassified as SV_BND_DUP")

        misclassified_inv = grouped['inv_variants'][
            grouped['inv_variants']['variant_type'] == 'SV_BND_INV'
        ] if len(grouped['inv_variants']) > 0 else []
        self.assertEqual(len(misclassified_inv), 0,
        ...
        for _, variant in true_bnd_variants.iterrows():
            self.assertIn(variant['variant_type'], ['SV_BND', 'SV_BND_INS'],
...
class TestExactOutputVerification(unittest.TestCase):
    ...
    def test_inv_exact_output(self):
        ...
        # Process inv.vcf
        personalized = sl.get_personal_genome(self.reference_fa, self.inv_vcf, encode=False)
    ...
    def test_dup_exact_output(self):
        ...
        # Process dup.vcf
        personalized = sl.get_personal_genome(self.reference_fa, self.dup_vcf, encode=False)
    ...
    def test_bnd_exact_output(self):
        ...
        # Process bnd.vcf
        personalized = sl.get_personal_genome(self.reference_fa, self.bnd_vcf, encode=False)
        ...
        for seq_name in expected.keys():
            self.assertIn(seq_name, personalized,
...
class TestProcessingIntegrity(unittest.TestCase):
    ...
    def test_inv_processing_details(self):
        ...
        self.assertGreater(len(grouped['inv_variants']), 0,
        ...
        self.assertEqual(len(symbolic_inv_variants), 1,
    ...
    def test_dup_processing_details(self):
        ...
        # Verify correct grouping
        self.assertGreater(len(grouped['dup_variants']), 0,
        ...
        self.assertEqual(len(symbolic_dup_variants), 1,
